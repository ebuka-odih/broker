                    ...
                    <ul>
                      <li class="d-flex justify-content-between align-items-center">
                        <div class="d-flex align-items-center">
                          <i style="color: #6c6cf3; font-size: 40px" class="icon ion-md-cash"></i>
                          <h4 class="ml-2"> Available Balance</h4>
                        </div>
                        <div>
                          <h4>{{ number_format($user->balance, 2 ?? '0') }} USD</h4>
                        </div>
                      </li>
                    </ul>
					...
					<div class="card-body p-2">
                       @forelse($wallets as $item)
                        <div id="accordion">
                            <div class="card my-2">
                                <div class="card-header">
                                    <a class="card-link d-flex align-items-center text-black collapsed" data-toggle="collapse" href="#collapse-{{ $item->id }}" aria-expanded="false">
                                        <div class="mr-2">
                                            <img src="{{ asset($item->avatar ?? 'img/wallet.png') }}" style="width: 30px; border-radius: 50%">
                                        </div>
                                ...
                                <div id="collapse-{{ $item->id }}" class="collapse" data-parent="#accordion">
                                    <div class="card-body">
                                        ...
                                        <div class="form-group">
                                            <input type="text" class="form-control" id="wallet-{{ $item->id }}" value="{{ $item->address }}" readonly>
                                        </div>
                                        <button class="btn btn-primary btn-block" onclick="copyFunction('{{ $item->id }}')">Tap to copy address</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Toast notification -->
                        <div id="toast-{{ $item->id }}" style="display: none; position: absolute; top: 20px; right: 20px; background-color: #4CAF50; color: white; padding: 10px 20px; border-radius: 5px; margin: 10px">
			...
			<div class="col-md-7 py-3">
				<div class="card my-2">
					...
					<div class="card-body px-3 py-5">
						<form action="{{ route('user.payment') }}" method="POST" enctype="multipart/form-data" class="mx-auto" style="max-width: 360px">
							@csrf
                            ...
                            @endif
							<p>To deposit, choose the payment method panel and make the payment to the displayed address. After payment has been made, come back to fill this form.</p>
							<div class="form-group">
								<label>Method</label>
